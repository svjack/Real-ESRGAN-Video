...
def save_frames_to_images(video_clip, output_dir):
...
def merge_images_to_video(input_dir, output_video_path, fps):
    ...
    sr_video_clip = ImageSequenceClip(frames, fps=fps)
    sr_video_clip.write_videofile(output_video_path, codec='libx264')

def upscale_video(input_video_path, output_video_path, upscale_factor=4):
    # 加载视频
    ...
    # 将视频帧保存为图片
    save_frames_to_images(video_clip, temp_dir_original)
...
if __name__ == "__main__":
    ...
    output_video_path = "1_spx4.mp4"
    upscale_factor = 4
    upscale_video(input_video_path, output_video_path, upscale_factor)
